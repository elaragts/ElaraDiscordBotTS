...
module.exports = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (client.ongoingBattles.has(userOne.id)) {
            await replyWithErrorMessage(interaction, 'Battle', 'You already started a battle');
            return;
        }
        //buttons
        ...
        if (baid === undefined) {
            await replyWithErrorMessage(interaction, 'Battle', 'You have not linked your discord account to your card yet!');
            return;
        }
        const nameOne = (await getMyDonName(baid))!;

        const songValidationResult = await validateSongInput(interaction, songInput!, "Battle");
        if (songValidationResult === undefined) return;
        const [uniqueId, lang] = songValidationResult;

        if (getSongStars(uniqueId, difficulty) === 0) {
            await replyWithErrorMessage(interaction, 'Battle', 'This song does not have a chart for this difficulty');
            return;
        }
